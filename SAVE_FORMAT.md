...
Статус: active schema-1 contract, implemented by Slice 2
Начальная schema: `1`  
...
- Невалидный Core binding сохраняет только quarantine marker вместо создания новой Village при следующей загрузке; repair выполняется явно либо заменой блока.
...
- `last_binding_generation` всегда положителен и сохраняется после удаления Core; успешный rebind увеличивает его. Это не позволяет stale local NBT стать active binding.
...
| `LevelEvent.Save` в 1.21.1 | Post-save verification/metrics: событие приходит после `DimensionDataStorage#save`; оно не является первым местом materialization или dirty marker и не создаёт новую metadata-only mutation. |
| Server stopping | Запрет новых jobs/commands, полный bookkeeping flush до shutdown world save, затем shutdown runtime/executors. |
...
1. Получить raw root NBT через `SavedData` loader. Factory deserializer не выпускает decode exception наружу: failure становится quarantined read-only `SyntValleySavedData`, потому что `DimensionDataStorage` 1.21.1 иначе трактует exception как missing data.
...
Quarantined instance хранит defensive copy исходного root, не помечается dirty, блокирует SyntValley mutations и возвращает исходный root при непредвиденном `save`. Это защищает future/corrupt schema от замены пустым состоянием; automatic record dropping не выполняется.
...
Закреплённые Slice 2 bounds: максимум `4096` villages в world state, village name — `64` Unicode code points, dimension id — `128` ASCII characters, binding generation — `1..2147483647`. Пока соответствующие record codecs не реализованы следующими slices, root collections `citizens/tasks/projects/memories/decisions` и village refs/zones/alerts обязаны присутствовать, но быть пустыми; non-empty значение переводит loader в quarantine вместо молчаливого отбрасывания.
